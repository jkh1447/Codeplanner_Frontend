...
export default function SummaryPage() {
    ...
    useEffect(() => {
        ...
        const fetchAllIssue = async () => {
            ...
            setInProgressIssue(
                data.filter((issue: any) => issue.status === "IN_PROGRESS")
                    .length
            );
            ...
            const progressDataArr = Object.entries(progressCount).map(
                ([progress, count]) => ({
                    id: progress,
            ...
            // 차트용 배열로 변환
            const typeDataArr = Object.entries(typeCount).map(
                ([type, count]) => ({
                    id: type,
                    value: count as number,
                })
        ...
        const fetchLabelData = async () => {
            const response = await fetch(
                `${getApiUrl()}/projects/${projectId}/labels-count`,
                {
                    method: "GET",
                    credentials: "include",
                }
                            ...
                            {recentActivities.length === 0 ? (
                                ...
                                recentActivities.map((activity) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div
                                                className={`w-2 h-2 ${style.color} rounded-full`}
                                            ></div>
