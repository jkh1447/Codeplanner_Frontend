...
export default function CommitListModal({
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (projectData.repository_url) {
                        const match = projectData.repository_url.match(
                            /github\.com\/([^\/]+)\/([^\/]+)/
                        );
                        if (match) {
    ...
    const handleShowFiles = async (sha: string) => {
        ...
        try {
            const res = await fetch(
                `${getApiUrl()}/github/repos/${projectOwner}/${projectRepo}/commit/${sha}/files`,
                { credentials: "include" }
            );
    ...
    const handleAnalyze = async (file: any) => {
        ...
        try {
            const res = await fetch(
                `${getApiUrl()}/analysis/github/commit/${projectOwner}/${projectRepo}/${
                    selectedCommit?.sha
                }?file=${file.filename}`,
                {
                    method: "GET",
            ...
            setAnalyzeResults((prev) => ({
                ...
                [file.filename]: {
                    ...
                    cppcheckOutput:
                        result?.cppcheck && !result?.cppcheck?.success
                            ? result.cppcheck.output
                            : undefined,
                    clangTidyOutput:
                        result?.clangTidy && !result?.clangTidy?.success
                            ? result.clangTidy.output
                            : undefined,
                    clangFormatOutput:
                        result?.clangFormat && !result?.clangFormat?.success
                            ? result.clangFormat.output
                            : undefined,
    ...
    const handleGetContent = async (file: CommitFile) => {
        ...
        try {
            const res = await fetch(
                `${getApiUrl()}/github/repos/commit/${projectOwner}/${projectRepo}/${sha}?file=${
                    file.filename
                }`,
                {
                    credentials: "include",
                    ...
                    <div className="mt-4 space-y-4 max-h-[60vh] overflow-y-auto">
                        {fileList.map((file: any, idx) => {
                            ...
                            if (result) {
                                ...
                                if (cppSuccess && formatSuccess)
                                    color = "bg-green-500";
                                else if (!cppSuccess && !formatSuccess)
                                    color = "bg-red-500";
                                    ...
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between mb-3">
                                                ...
                                                <span
                                                    className={`px-3 py-1 rounded-full text-xs font-medium border ${statusColor} flex-shrink-0`}
                                                >
                                                    {file.status}
                                                </span>
                                            </div>
                                            ...
                                            {fileContents[file.filename] && (
                                                <div className="mt-4">
                                                    ...
                                                    {(() => {
                                                        ...
                                                        // cppcheck 에러 라인 추출
                                                        let cppcheckErrorLines =
                                                            new Set<number>();
                                                        if (
                                                            result &&
                                                            result.cppcheckIssues
                                                        ) {
                                                            cppcheckErrorLines =
                                                                new Set(
                                                                    result.cppcheckIssues.map(
                                                                        (
                                                                            issue: any
                                                                        ) =>
                                                                            issue.line
                                                                ...
                                                                {codeLines.map(
                                                                    (
                                                                        ...
                                                                        const lineNumber =
                                                                            idx +
                                                                            1;
                                                                        const isCppcheckError =
                                                                            cppcheckErrorLines.has(
                                                                                lineNumber
                                                                            );
                                                                        return (
                                                                            <div
                                                                                ...
                                                                                className="flex"
                                                                            >
                                                                                <span className="w-8 text-right pr-2 select-none text-gray-400">
                                                                                    {
                                                                                        lineNumber
                                                                                    }
                                                                                </span>
